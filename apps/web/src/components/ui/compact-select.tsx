...
export function CompactSelect({
  ...
  return (
    <div ref={rootRef} className={cn("relative mt-2", className)}>
      {/* Single hidden input — always in DOM, always submitted */}
      <input type="hidden" name={name} value={value} />

      {/* Mobile: native <select> triggers platform picker (iOS wheel, Android list) */}
      <div className="relative sm:hidden">
        <select
          value={value}
          onChange={(e) => setValue(e.target.value)}
          className="w-full appearance-none rounded-2xl border border-[rgba(123,167,209,0.24)] bg-white/72 px-4 py-3 pr-11 text-sm font-medium text-[var(--text-strong)] outline-none transition-[border-color,box-shadow,background-color] focus:border-[var(--accent)] focus:bg-white/88 focus:shadow-[0_0_0_4px_rgba(77,168,218,0.14)]"
        >
          {options.map((option) => (
